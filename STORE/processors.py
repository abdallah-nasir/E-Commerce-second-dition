    ...
    if request.user.is_authenticated:    
        ...
        try:
            ...
            if Wishlist.objects.filter(device=device,user=None).exists():
                wishlist=Wishlist.objects.filter(device=device,user=None).latest("modified_date")
                wishlist.user=request.user
                wishlist.save()  
            lists=Wishlist.objects.filter(user=request.user,device=device)
            if len(lists) > 1: 
                  
                for i in Wishlist.objects.filter(user=request.user,device=device).exclude(id=lists.latest("modified_date").id):
                    i.delete()
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            if Cart.objects.filter(device=device,user=None,ordered=True,delivered=False).exists():
                cart=Cart.objects.filter(device=device,user=None,ordered=True,delivered=False).latest("modified_date")
            ...
            if len(carts) > 1:
                for i in Cart.objects.filter(user=request.user,device=device,ordered=True,delivered=False).exclude(id=carts.latest("modified_date").id):
                    i.delete()
